<template>
    <div class="wordList-content" v-loading="loading">
        <div class="wordItem" v-for="AllLearnedWords in this.WordsListRst" :key="AllLearnedWords.id">
            <p class="wordItem-word">
                <span>{{AllLearnedWords.words}}</span>
                <i class="bi bi-trash" @click="NoLearned(AllLearnedWords.id)"></i>
                <el-tooltip :content="AllLearnedWords.phonetic" placement="right" effect="light">
                    <i class="bi bi-volume-down-fill"></i>
                </el-tooltip>
            </p>
            <p class="wordItem-tran"><strong>{{AllLearnedWords.translation}}</strong><i class="bi bi-chevron-down" @click="downUp($event)"></i></p>
            <p class="wordItem-details" style="display: none;">{{AllLearnedWords.notes}}</p>
        </div>
    </div>
</template>
...
export default {
    ...
    async mounted() {
         try {
            await this.$store.dispatch('word/GetAllLearnedWords');
            this.loading = false
        } catch (error) {
            this.loading = false
            console.log(error);
        ...
        // wordlist()
        this.WordsList = this.AllLearnedWords
        this.WordsListRst = this.AllLearnedWords
    },
    computed:{
		...mapState('word',['AllLearnedWords'])
	},
    methods: {
        NoLearned(id){
            this.$confirm('是否确定移除该单词？','提示',{
                ...
                await this.$store.dispatch('word/UpdateUserWordNotLearned',{id});
                this.$message({
                    type:'success',
                    message: '移除成功！',
                    duration:2500,
